/*=====================================================================
  script.js – FULL: DYNAMIC ROUND-ROBIN + AUTO-KNOCKOUT + GITHUB AUTO-LOAD
=====================================================================*/
...
async function autoLoadFromGitHub() {
  ...
  try {
    ...
    console.warn('GitHub auto-load failed (using local):', e.message);
  }
}
...
function initMatches() {
  ...
  matchesDiv.addEventListener('click', e => {
    ...
    if (editTimeId) {
      ...
      const newTime = prompt('New time (HH:MM):', timeStr);
      if (newDate && newTime) {
...
/*=====================================================================
  KNOCKOUT – FIXED: HEADERS + PERFECT ALIGNMENT + SCORE INPUTS
=====================================================================*/
  ...
  const render = () => {
    ...
    let html = `
      <style>
        ...
        .match-box { cursor: pointer; }
        .score-input { 
          width: 40px; height: 32px; text-align: center; font-weight: bold; 
          border: 1px solid #ffd700; border-radius: 4px; background: #111; color: #fff;
        }
      </style>
...
function initDashboard() {
  ...
  ghSection.innerHTML = `
    ...
    <div class="grid grid-3">
      <div><label>Repo</label><input id="ghRepo" value="${GITHUB_REPO}" placeholder="imitrr/Rosebel-Street-Soccer"></div>
    ...
    <p style="font-size:0.8rem; margin-top:0.5rem; color:#aaa;">
      Data auto-loads from GitHub on every page. Only admins need token to save.
    </p>`;
