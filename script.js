...
// ---------------------------------------------------------------------
// AUTO-LOAD FROM GITHUB (FIX: TOAST ONLY ON CHANGE)
async function autoLoadFromGitHub() {
  ...
  const snapshot = {
    ...
    bracket: JSON.stringify(getBracket())
  };
  try {
    ...
    ['teams','players','matches','bracket'].forEach(k => {
      if (data[k] !== undefined && JSON.stringify(data[k]) !== snapshot[k]) {
        store.set(k, data[k]);
        changed = true;
      }
    });
    if (changed) toast('Data updated from GitHub', 'success');
    // Silent if no change
  } catch (e) {
...
/*=====================================================================
  PLAYERS PAGE (FIX: 40PX PHOTOS, STATS DISPLAYED/EDITABLE)
=====================================================================*/
if (page === 'players') {
      ...
      return `
        <div class="team-block">
          ...
          <div style="display: grid; gap: 0.5rem;">
            ${ps.map(p => `
              <div class="player-card" style="display: flex; align-items: center; gap: 0.5rem; padding: 0.5rem; border-radius: 8px; background: rgba(0,0,0,.4);">
                <img src="${p.photo || ''}" alt="${p.name}" style="width: 40px; height: 40px; border-radius: 50%; object-fit: cover; border: 2px solid #ffd700;" onerror="this.src='data:image/svg+xml;base64,'" />
                <div class="number" style="font-weight: bold; color: #ffd700;">${p.number || ''}</div>
                <div class="info" style="flex: 1;">
                  <strong>${p.name}</strong><br>
                  <small>${p.position || ''}</small>
                </div>
                <div class="stats" style="font-size: 0.8rem; color: #aaa; text-align: right; min-width: 120px;">
                  G: ${p.goals || 0} | A: ${p.assists || 0}<br>
                  YC: ${p.yellowCards || 0} | RC: ${p.redCards || 0}
                </div>
                <div class="actions" style="display: flex; gap: 0.25rem;">
                  <button class="btn sm primary" onclick="editPlayer('${teamId}', ${ps.indexOf(p)})">Edit</button>
  ...
  window.editPlayer = (teamId, idx) => {
    ...
    byId('editPlayerGoals').value = p.goals || 0;
    byId('editPlayerAssists').value = p.assists || 0;
    byId('editPlayerYellowCards').value = p.yellowCards || 0;
    byId('editPlayerRedCards').value = p.redCards || 0;
  ...
  form.onsubmit = async e => {
    ...
    players[teamId].push({ name, number, position, photo, goals: 0, assists: 0, yellowCards: 0, redCards: 0 });
    setPlayers(players);
    form.reset();
    renderPlayers();
  ...
  editForm.innerHTML = `
    ...
    <div><label for="editPlayerName">Name</label><input id="editPlayerName" required></div>
    <div><label for="editPlayerNumber"># Number</label><input id="editPlayerNumber" type="number" min="1" max="99"></div>
    <div><label for="editPlayerPosition">Position</label>
    ...
    <div><label for="editPlayerGoals">Goals</label><input id="editPlayerGoals" type="number" min="0"></div>
    <div><label for="editPlayerAssists">Assists</label><input id="editPlayerAssists" type="number" min="0"></div>
    <div><label for="editPlayerYellowCards">Yellow Cards</label><input id="editPlayerYellowCards" type="number" min="0"></div>
    <div><label for="editPlayerRedCards">Red Cards</label><input id="editPlayerRedCards" type="number" min="0"></div>
    <div><label for="editPlayerPhoto">Photo</label><input id="editPlayerPhoto" type="file" accept="image/*"></div>
    <div class="full actions">
      <button type="button" class="btn sm" id="closePlayerModal">Cancel</button>
  ...
  editForm.onsubmit = async e => {
    ...
    const goals = +byId('editPlayerGoals').value || 0;
    const assists = +byId('editPlayerAssists').value || 0;
    const yellowCards = +byId('editPlayerYellowCards').value || 0;
    const redCards = +byId('editPlayerRedCards').value || 0;
    ...
    const players = getPlayers();
    players[teamId][idx] = { ...players[teamId][idx], name, number, position, goals, assists, yellowCards, redCards, photo };
    setPlayers(players);
    modal.style.display = 'none';
    renderPlayers();
...
/*=====================================================================
  MATCHES PAGE (FIX: FALLBACK DATE/TIME, BETTER BUTTONS)
=====================================================================*/
if (page === 'matches') {
  ...
  function renderMatches() {
    const matches = getMatches().sort((a,b) => new Date(`${a.date || '1970-01-01'}T${a.time || '00:00'}`) - new Date(`${b.date || '1970-01-01'}T${b.time || '00:00'}`)); // Sort by time, fallback
    container.innerHTML = matches.map((m, idx) => {
      ...
      const dateTime = (m.date && m.time) ? `${m.date} ${m.time}` : 'TBD';
      return `
        <div class="match-row" style="display: flex; flex-wrap: wrap; gap: 0.5rem; align-items: center; padding: 1rem; border: 1px solid #ffd700; border-radius: 8px; margin-bottom: 0.5rem; justify-content: space-between;">
          <div style="display: flex; align-items: center; gap: 0.5rem; flex: 1;">
            <img src="${home.logo || ''}" alt="${home.name}" style="width: 24px; height: 24px; border-radius: 50%; border: 2px solid #ffd700;" onerror="this.src='data:image/svg+xml;base64,'" />
            <span style="font-weight: bold;">${home.name}</span>
          </div>
          <div style="display: flex; align-items: center; gap: 0.5rem; font-size: 1.2rem;">
            <input class="score-input" data-id="${m.id}" data-side="home" value="${m.homeScore || 0}" min="0" step="1" type="number" style="width: 40px; text-align: center; border-radius: 4px;">
            <span>-</span>
            <input class="score-input" data-id="${m.id}" data-side="away" value="${m.awayScore || 0}" min="0" step="1" type="number" style="width: 40px; text-align: center; border-radius: 4px;">
          </div>
          <div style="display: flex; align-items: center; gap: 0.5rem; flex: 1; justify-content: flex-end;">
            <span style="font-size: 0.8rem; color: #aaa;">${dateTime}</span>
            <img src="${away.logo || ''}" alt="${away.name}" style="width: 24px; height: 24px; border-radius: 50%; border: 2px solid #ffd700;" onerror="this.src='data:image/svg+xml;base64,'" />
            <span style="font-weight: bold;">${away.name}</span>
          </div>
          <button class="btn sm primary" data-edit="${m.id}" style="margin-top: 0.5rem; width: 100%;">Edit Time/Date</button>
        </div>
      `;
    }).join('');
    // Re-attach events
...
/*=====================================================================
  STANDINGS PAGE (FIX: SCROLLABLE ON MOBILE, BETTER ALIGNMENT)
=====================================================================*/
if (page === 'standings') {
  ...
  function renderStandings() {
    ...
    if (teams.length === 0) {
      container.innerHTML = '<p style="text-align:center;color:#aaa;">No teams yet. Add teams and matches to see standings.</p>';
      return;
    }

    // Calculate full stats per team
    const stats = {};
    teams.forEach(t => {
      stats[t.id] = { p: 0, w: 0, d: 0, l: 0, gf: 0, ga: 0, pts: 0 };
    });

    // Count matches played, wins, etc.
    const played = {}; // Track opponents to avoid double-count
    matches.forEach(m => {
      const hId = m.homeId, aId = m.awayId;
      if (!stats[hId] || !stats[aId]) return;
      if (!played[hId]) played[hId] = new Set();
      if (!played[aId]) played[aId] = new Set();
      if (!played[hId].has(aId)) {
        stats[hId].p++; stats[aId].p++;
        played[hId].add(aId); played[aId].add(hId);
      }
      stats[hId].gf += m.homeScore; stats[hId].ga += m.awayScore;
      stats[aId].gf += m.awayScore; stats[aId].ga += m.homeScore;
      const hScore = m.homeScore, aScore = m.awayScore;
      if (hScore > aScore) { stats[hId].w++; stats[aId].l++; stats[hId].pts += 3; }
      else if (hScore < aScore) { stats[aId].w++; stats[hId].l++; stats[aId].pts += 3; }
      else { stats[hId].d++; stats[aId].d++; stats[hId].pts++; stats[aId].pts++; }
    });

    Object.values(stats).forEach(s => s.gd = s.gf - s.ga);

    const ranked = teams.map(t => ({ ...stats[t.id], ...t, pos: 0 })).sort((a, b) => 
      b.pts - a.pts || b.gd - a.gd || b.gf - a.gf
    );
    ranked.forEach((t, i) => t.pos = i + 1); // Positions

    container.innerHTML = `
      <div style="overflow-x: auto;">
        <table class="standings-table" style="width: 100%; border-collapse: collapse; font-size: 0.9rem;">
          <thead>
            <tr style="background: #ffd700; color: #000; text-align: center; font-weight: bold;">
              <th>Pos</th><th>Team</th><th>P</th><th>W</th><th>D</th><th>L</th><th>GF</th><th>GA</th><th>GD</th><th>PTS</th>
            </tr>
          </thead>
          <tbody>
            ${ranked.map((t, i) => `
              <tr style="background: ${i % 2 ? 'rgba(255,215,0,0.1)' : 'transparent'}; text-align: center;">
                <td style="font-weight: bold;">${t.pos}</td>
                <td style="display: flex; align-items: center; gap: 0.5rem; justify-content: flex-start; padding: 0.5rem;">
                  <img src="${t.logo || ''}" alt="${t.name}" style="width: 28px; height: 28px; border-radius: 50%; border: 2px solid #ffd700;" onerror="this.src='data:image/svg+xml;base64,'" />
                  <span>${t.name}</span>
                </td>
                <td>${t.p}</td><td>${t.w}</td><td>${t.d}</td><td>${t.l}</td><td>${t.gf}</td><td>${t.ga}</td>
                <td style="color: ${t.gd >= 0 ? '#2e7d32' : '#d32f2f'}; font-weight: bold;">${t.gd}</td>
                <td style="font-weight: bold; color: #ffd700;">${t.pts}</td>
              </tr>
            `).join('')}
          </tbody>
        </table>
...
/*=====================================================================
  KNOCKOUT PAGE (FIX: BETTER SCORE INPUTS WITH STEP=1, MOBILE SCROLL)
=====================================================================*/
if (page === 'knockout') {
  ...
  function updateWinners() {
    const b = getBracket();
    if (b.rounds.length < 2) return; // Need at least semis + final
    // Update final from semis
    const semis = b.rounds[0]; // Assuming rounds[0] = semis
    if (semis.length === 2) {
      const final = b.rounds[1][0];
      const w1 = semis[0].homeScore > semis[0].awayScore ? semis[0].homeId : 
                 semis[0].homeScore < semis[0].awayScore ? semis[0].awayId : null;
      const w2 = semis[1].homeScore > semis[1].awayScore ? semis[1].homeId : 
                 semis[1].homeScore < semis[1].awayScore ? semis[1].awayId : null;
  ...
  const render = () => {
    ...
    if (!rounds.length) { 
      container.innerHTML = '<p style="text-align:center;color:#aaa;">Generate bracket from standings (top 4 teams seeded: 1v4, 2v3 semis).</p>'; 
      return; 
    }

    const roundLabels = ['Semi-Finals', 'Final']; // For one-day: Semis → Final
    const teamCount = rounds[0].length * 2;
    const roundCount = rounds.length;
    const width = 320 + roundCount * 340;
    const height = Math.max(500, teamCount * 90 + 100); // Tighter for small brackets
    ...
    const sx = 140, sy = 150, rg = 340, vg = 180; // Vertical spacing for semis/final
    rounds.forEach((r, ri) => {
      const x = sx + ri * rg;
      html += `<text x="${x+140}" y="${sy-40}" class="round-header">${roundLabels[ri] || 'Round ' + (ri + 1)}</text>`;
      r.forEach((m, i) => {
        const y = sy + i * vg;
        const home = getTeams().find(t => t.id === m.homeId) || {name: 'TBD', logo: ''};
        const away = getTeams().find(t => t.id === m.awayId) || {name: 'TBD', logo: ''};
        ...
        html += `<g class="match">
          ...
          <foreignObject x="${x+225}" y="${y+14}" width="44" height="32">
            <input class="score-input" data-id="${m.id}" data-side="home" value="${m.homeScore||0}" type="number" min="0" step="1"/>
          </foreignObject>`;
        ...
        html += `<rect x="${x}" y="${ay}" width="280" height="60" rx="12" fill="#1a1a1a" stroke="#ffd700" stroke-width="2"/>
          ...
          <foreignObject x="${x+225}" y="${ay+14}" width="44" height="32">
            <input class="score-input" data-id="${m.id}" data-side="away" value="${m.awayScore||0}" type="number" min="0" step="1"/>
          </foreignObject>`;
    ...
    // Score change handler with auto-advance
    container.querySelectorAll('.score-input').forEach(inp => {
      inp.addEventListener('change', e => {
        const id = e.target.dataset.id;
        const side = e.target.dataset.side;
  ...
  genBtn.onclick = () => {
    const matches = getMatches();
    const teams = getTeams();
    if (teams.length < 4) return toast('Need 4+ teams for semis', 'error');

    // Use standings logic for ranking
    const stats = {};
    ...
    Object.values(stats).forEach(s=>s.gd=s.gf-s.ga);
    const ranked = teams.map(t=>({...stats[t.id],name:t.name,logo:t.logo,id:t.id}))
                       .sort((a,b)=>b.pts-a.pts||b.gd-a.gd||b.gf-a.gf);

    // Take top 4 (or 8 for quarters); pro seeding
    const topCount = ranked.length >= 8 ? 8 : ranked.length >= 4 ? 4 : 0;
    if (topCount < 4) return toast('Need matches played for proper ranking', 'error');
    ...
    if (topCount === 8) {
      ...
      const quarters = [
        {id: uuid(), homeId: top[0].id, awayId: top[7].id, homeScore:0, awayScore:0},
        {id: uuid(), homeId: top[3].id, awayId: top[4].id, homeScore:0, awayScore:0},
        {id: uuid(), homeId: top[1].id, awayId: top[6].id, homeScore:0, awayScore:0},
        {id: uuid(), homeId: top[2].id, awayId: top[5].id, homeScore:0, awayScore:0}
      ];
      rounds.push(quarters);
      // Semis from quarters (logic in updateWinners)
      rounds.push([{id: uuid(), homeId: null, awayId: null, homeScore:0, awayScore:0}, {id: uuid(), homeId: null, awayId: null, homeScore:0, awayScore:0}]);
      rounds.push([{id: uuid(), homeId: null, awayId: null, homeScore:0, awayScore:0}]); // Final
    } else {
      // Semis: 1v4, 2v3
      const semis = [
        {id: uuid(), homeId: top[0].id, awayId: top[3].id, homeScore:0, awayScore:0},
        {id: uuid(), homeId: top[1].id, awayId: top[2].id, homeScore:0, awayScore:0}
      ];
      rounds.push(semis);
      rounds.push([{id: uuid(), homeId: null, awayId: null, homeScore:0, awayScore:0}]); // Final
    }
    setBracket({rounds});
    render();
    toast(`Bracket generated for top ${topCount} teams`, 'success');
  };
